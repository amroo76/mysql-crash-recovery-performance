...
dt=`date +%Y%m%d%H%M%S`
logdir="/home/amrendra/projects/crash-recovery/log/writeonly"
threads=10
...
$BIN_DIR/mysql --host=localhost -uroot --get-server-public-key --socket=/home/amrendra/projects/crash-recovery/mysql.sock -p.userMDS00 -vvv -e " SHOW ENGINE INNODB STATUS;set global innodb_checkpoint_disabled=1;" >  ${logdir}/${dt}_wrt_100k_before_test.log

$BMK_HOME/sysbench-1.1-mysql80-ssl111L-x64 $BMK_HOME/sb_exec/lua/oltp_write_only.lua   --db-driver=mysql --mysql-host=localhost --events=100000 --time=300 --mysql-db=test --mysql-password=.userMDS00 --mysql-socket=/home/amrendra/projects/crash-recovery/mysql.sock --mysql-user=dim --threads=${threads} --rand-type=uniform --tables=8 --forced-shutdown=1 --table-size=500000 --auto-inc=on --mysql-ssl=disabled --thread-init-timeout=0 run > ${logdir}/${dt}_key_100k_sysb.log
sleep 10
$BIN_DIR/mysql --host=localhost -uroot --get-server-public-key --socket=/home/amrendra/projects/crash-recovery/mysql.sock -p.userMDS00 -vvv -e " SHOW ENGINE INNODB STATUS;" > ${logdir}/${dt}_wrt_100k_after_test.log
...
$BIN_DIR/mysql --host=localhost -uroot --get-server-public-key --socket=/home/amrendra/projects/crash-recovery/mysql.sock -p.userMDS00 -vvv -e " SHOW ENGINE INNODB STATUS;" >  ${logdir}/${dt}_wrt_100k_after_crash.log
sleep 10


cp /home/amrendra/projects/crash-recovery/data/mysqld_error.log ${logdir}/${dt}_wrt_100k_error.log
