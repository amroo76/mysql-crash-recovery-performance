...
dt=`date +%Y%m%d%H%M%S`
logdir="/home/amrendra/projects/crash-recovery/log/nokey-mod"
mkdir -p $logdir
threads=10
...
$BIN_DIR/mysql --host=localhost -uroot --get-server-public-key --socket=/home/amrendra/projects/crash-recovery/mysql.sock -p.userMDS00   -e  "ALTER USER 'root'@'localhost' IDENTIFIED BY '.userMDS00';"
...
$BMK_HOME/sysbench-1.1-mysql80-ssl111L-x64 $BMK_HOME/sb_exec/lua/OLTP_RW-non_index_updates-trx-mod.lua   --db-driver=mysql --mysql-host=localhost --events=100000 --time=300 --mysql-db=test --mysql-password=.userMDS00 --mysql-socket=/home/amrendra/projects/crash-recovery/mysql.sock --mysql-user=dim --threads=${threads} --rand-type=uniform --tables=8 --forced-shutdown=1 --skip-trx=off --table-size=500000 --auto-inc=on --mysql-ssl=disabled --thread-init-timeout=0 run > ${logdir}/${dt}_key_100k_sysb.log
sleep 10
